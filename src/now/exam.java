...
public class exam {
	...
	// solve brackets if no found or finish and still expression go to
	// dooperation
		...
		while (X.contains(Character.toString('(')) || X.contains(Character.toString(')'))) {
			for (int I = 0; I < X.length(); I++) {
				try { // i there is not sign
					if ((X.charAt(I) == ')' || Character.isDigit(X.charAt(I))) // between
																				// separate
																				// brackets
							&& X.charAt(I + 1) == '(') { // or number and
															// bracket,
						X = X.substring(0, I + 1) + "*" + (X.substring(I + 1)); // it
																				// treat
																				// it
																				// as
					} // a multiplication
				...
				if (X.charAt(I) == ')') { // search for a closing bracket
					for (int i = I; i >= 0; i--) {
						if (X.charAt(i) == '(') { // search for a opening
													// bracket
